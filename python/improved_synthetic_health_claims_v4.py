...
# cd "C:\Users\jakeq\OneDrive\Documents\GitHub\big_data_and_innovation"
# python .\improved_synthetic_health_claims_v4.py --n_encounters 3000000 --n_members 1200000 --n_providers 10000 --chunk_size 100000
# 04/01/2026
...
PROFESSIONAL_PROCEDURES = pd.DataFrame([
    ...
    ("27447", 1850.0, 1, "Procedure"), ("47562", 1325.0, 1, "Procedure"),
    ("99221", 165.0, 1, "Hospital E&M"), ("99222", 220.0, 1, "Hospital E&M"), ("99223", 285.0, 1, "Hospital E&M"),
    ("99231", 75.0, 1, "Hospital E&M"), ("99232", 110.0, 1, "Hospital E&M"), ("99233", 155.0, 1, "Hospital E&M"),
    ("74177", 260.0, 1, "Imaging"), ("70450", 175.0, 1, "Imaging"), ("72148", 410.0, 1, "Imaging"),
    ("00142", 620.0, 1, "Anesthesia"), ("00840", 760.0, 1, "Anesthesia"), ("01402", 940.0, 1, "Anesthesia"), ("01967", 850.0, 1, "Anesthesia"),
    ("88305", 115.0, 1, "Pathology"), ("88342", 135.0, 1, "Pathology"),
    ("97530", 48.0, 2, "Therapy"), ("97165", 115.0, 1, "Therapy"), ("97166", 135.0, 1, "Therapy"),
    ("97535", 52.0, 2, "Therapy"), ("92507", 98.0, 1, "Therapy"), ("92523", 145.0, 1, "Therapy"), ("92610", 160.0, 1, "Therapy"),
...
SPECIALTY_TO_CODES = {
    ...
    "Surgery": np.array(["99221", "99222", "99223", "99231", "99232", "99233", "47562", "27447", "12002"], dtype=object),
    "Radiology": np.array(["71046", "74177", "70450", "72148", "93000"], dtype=object),
    "Hospitalist": np.array(["99221", "99222", "99223", "99231", "99232", "99233"], dtype=object),
    "Anesthesiology": np.array(["00142", "00840", "01402", "01967"], dtype=object),
    "Pathology": np.array(["88305", "88342", "85025", "80053"], dtype=object),
    "Physical Therapy": np.array(["97110", "97112", "97140", "97530"], dtype=object),
    "Occupational Therapy": np.array(["97165", "97166", "97535", "97530"], dtype=object),
    "Speech Therapy": np.array(["92507", "92523", "92610"], dtype=object),
}
...
def make_providers(n_providers: int, rng: np.random.Generator) -> pd.DataFrame:
    specialties = np.array([
        ...
        "Surgery", "Radiology", "Hospitalist", "Anesthesiology", "Pathology",
        "Physical Therapy", "Occupational Therapy", "Speech Therapy"
    ], dtype=object)
    probs = np.array([0.14, 0.12, 0.06, 0.06, 0.06, 0.03, 0.07, 0.11, 0.04, 0.02, 0.07, 0.06, 0.05, 0.03, 0.03, 0.02, 0.02, 0.01])
    ...
    # Most providers are professional. Facilities remain a separate type for hospital and DME entities.
...
def choose_providers_by_specialty(providers: pd.DataFrame, specialties: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    provider_by_specialty = {
        sp: grp["provider_id"].to_numpy(dtype=np.int32)
        for sp, grp in providers.groupby("specialty", observed=True)
    }
    result = np.empty(len(specialties), dtype=np.int32)
    fallback = providers.loc[providers["provider_type"] == "professional", "provider_id"].to_numpy(dtype=np.int32)
    for sp in np.unique(specialties):
        idx = np.where(specialties == sp)[0]
        choices = provider_by_specialty.get(sp, fallback)
        result[idx] = rng.choice(choices, size=len(idx))
    return result


def build_linked_professional_encounters(base_encounters: pd.DataFrame, providers: pd.DataFrame, next_encounter_id: int, rng: np.random.Generator) -> pd.DataFrame:
    inst = base_encounters.loc[base_encounters["encounter_type"] == "institutional"].copy()
    if inst.empty:
        return pd.DataFrame(columns=base_encounters.columns)

    companion_rows = []
    for row in inst.itertuples(index=False):
        companion_specialties: list[str] = []
        if str(row.place_of_service) == "21":
            companion_specialties.append("Hospitalist")
            if row.planned_lines >= 12:
                companion_specialties.append("Surgery")
            if rng.random() < 0.55:
                companion_specialties.append("Radiology")
            if row.planned_lines >= 14 and rng.random() < 0.60:
                companion_specialties.append("Anesthesiology")
            if row.planned_lines >= 9 and rng.random() < 0.30:
                companion_specialties.append("Pathology")
            if row.planned_lines >= 16 and rng.random() < 0.40:
                therapy_n = 1 + int(row.planned_lines >= 24 and rng.random() < 0.35)
                therapy_choices = rng.choice(
                    np.array(["Physical Therapy", "Occupational Therapy", "Speech Therapy"], dtype=object),
                    size=therapy_n,
                    replace=False,
                )
                companion_specialties.extend(therapy_choices.tolist())
        elif str(row.place_of_service) == "23":
            companion_specialties.append("Radiology")
            if rng.random() < 0.45:
                companion_specialties.append("Hospitalist")
        else:
        ...
        for sp in companion_specialties[:6]:
            companion_rows.append({
                "member_id": row.member_id,
                "service_date": row.service_date,
                "encounter_type": "professional",
                ...
                "high_risk_member": row.high_risk_member,
                "specialty": sp,
                "provider_type": "professional",
                "provider_class": "clean",
                "provider_fraud_propensity": 0.0,
                "contract_rate_id": row.contract_rate_id if row.contract_rate_id != "OON" else "COMM_A",
                "planned_lines": int(rng.integers(1, 4 if "Therapy" in sp else 3)),
                "parent_encounter_id": int(row.encounter_id),
                "episode_id": int(row.episode_id),
                "linked_facility_claim": 1,
            })

    if not companion_rows:
        return pd.DataFrame(columns=base_encounters.columns)

    linked = pd.DataFrame(companion_rows)
    linked["encounter_id"] = np.arange(next_encounter_id, next_encounter_id + len(linked), dtype=np.int64)
    linked["provider_id"] = choose_providers_by_specialty(providers, linked["specialty"].to_numpy(dtype=object), rng)
    return linked
...
def generate_encounters(start_id: int, n_encounters: int, members: pd.DataFrame, providers: pd.DataFrame, rng: np.random.Generator) -> pd.DataFrame:
    ...
    encounter_type = rng.choice(
        ...
        size=n_encounters,
        p=[0.54, 0.20, 0.16, 0.06, 0.04],
    )

    hosp_mask = p["specialty"].to_numpy(dtype=object) == "Hospital"
    encounter_type[hosp_mask] = rng.choice(np.array(["institutional", "administrative", "pharmacy"], dtype=object), size=hosp_mask.sum(), p=[0.80, 0.14, 0.06])
    ...
    pos[encounter_type == "professional"] = rng.choice(np.array(["11", "22", "02", "23"], dtype=object), size=(encounter_type == "professional").sum(), p=[0.58, 0.16, 0.08, 0.18])
    pos[encounter_type == "institutional"] = rng.choice(np.array(["21", "22", "23"], dtype=object), size=(encounter_type == "institutional").sum(), p=[0.34, 0.28, 0.38])
    ...
    if inst.any():
        ...
        line_counts = np.where(
            inst_pos == "21",
            rng.integers(10, 36, inst.sum()),
            np.where(inst_pos == "23", rng.integers(4, 15, inst.sum()), rng.integers(3, 10, inst.sum()))
        )
    ...
    df = pd.DataFrame({
        ...
        "planned_lines": lines,
        "parent_encounter_id": np.zeros(n_encounters, dtype=np.int64),
        "episode_id": encounter_ids,
        "linked_facility_claim": np.zeros(n_encounters, dtype=np.int8),
    })

    linked = build_linked_professional_encounters(df, providers, int(df["encounter_id"].max()) + 1, rng)
    if not linked.empty:
        linked = linked[df.columns]
        linked_provider_meta = providers.set_index("provider_id").loc[linked["provider_id"].to_numpy(dtype=np.int32)]
        linked["provider_class"] = linked_provider_meta["provider_class"].to_numpy(dtype=object)
        linked["provider_fraud_propensity"] = linked_provider_meta["fraud_propensity"].to_numpy(dtype=np.float32)
        linked["contract_rate_id"] = linked_provider_meta["contract_rate_id"].to_numpy(dtype=object)
        df = pd.concat([df, linked], ignore_index=True)
...
def expand_to_lines(encounters: pd.DataFrame, rng: np.random.Generator) -> pd.DataFrame:
    ...
    df = lines[[
        ...
        "parent_encounter_id", "episode_id", "linked_facility_claim"
    ]].copy()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate large synthetic healthcare claim lines with linked facility and professional episodes, pharmacy, admin, denials, edits, and pricing anomalies.")
