...
def build_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["rule_provider_volume"] = (out["provider_day_claim_count"] >= 40).astype(np.int8)
    out["rule_egregious_billed"] = (out["billed_to_contract_ratio"] >= 6.0).astype(np.int8)
    out["rule_price_above_contract"] = (out["contract_to_allowed_ratio"] >= 1.25).astype(np.int8)
    out["rule_external_edit"] = out["external_edit_flag"].astype(np.int8)
    ...
    out["flagged_for_review"] = (out["rule_score"] >= 2).astype(np.int8)
    return out
...
def summarize(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    method_summary = pd.DataFrame({
        "metric": [
            "rows", "unique_members", "unique_providers", "unique_encounters", "fraud_rate",
            "flag_rate", "denial_rate", "pharmacy_share", "institutional_share", "linked_facility_professional_share"
        ],
        "value": [
            len(df), df["member_id"].nunique(), df["provider_id"].nunique(), df["encounter_id"].nunique(),
            round(df["fraud_label"].mean(), 6), round(df["flagged_for_review"].mean(), 6),
    ...
    rule_reason = df.groupby(["rule_primary_reason", "flagged_for_review"]).size().reset_index(name="count").sort_values("count", ascending=False)
...
def generate_chunk(start_encounter: int, n_encounters: int, members: pd.DataFrame, providers: pd.DataFrame) -> pd.DataFrame:
    ...
    ordered = [
        ...
        "rule_external_edit", "rule_denied_repeat", "rule_score", "flagged_for_review",
        "fraud_label", "fraud_type", "rule_primary_reason"
    ]
    for col in ordered:
        if col not in df.columns:
            df[col] = np.nan
